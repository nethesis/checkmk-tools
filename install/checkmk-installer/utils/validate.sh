...
# Check required commands  local required_commands=("curl" "wget" "jq" "systemctl" "apt-get")  for cmd in "${required_commands[@]}"; do    if command -v "$cmd" &>/dev/null; then      print_success "Comando '$cmd': OK"    else      print_warning "Comando '$cmd': NON TROVATO (verr├á installato)"    fi  done    return $errors}
...
echo "$ip"      return 0    else      print_error "Indirizzo IP non valido"    fi  done}input_port() {  local prompt="$1"  local default="$2"    while true; dolocal portlocal portport=$(input_text "$prompt" "$default" "^[0-9]+$")    if validate_port "$port"; then      
...
echo "$url"      return 0    else      print_error "URL non valido (deve iniziare con http:// o https://)"    fi  done}input_hostname() {  local prompt="$1"  local default="$2"    while true; dolocal hostnamelocal hostnamehostname=$(input_text "$prompt" "$default")    if validate_hostname "$hostname"; then      
echo "$hostname"      return 0    else      print_error "Hostname non valido"    fi  done}
