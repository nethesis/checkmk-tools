...
if [[ ! -r "$YUM_LOG" ]]; then
    ...
    exit 2fimkdir -p /var/lib/check_pkg_install >/dev/null 2>&1 || trueevent_line=$(grep -E 'Installed:|Updated:|Erased:|Removed:' "$YUM_LOG" | tail -n 1)
if [[ -z "$event_line" ]]; then
    echo "0 PKG_INSTALL - OK: nessuna attivit├â┬á pacchetti"
    exit 0fievent_date=$(
echo "$event_line" | awk '{print $1, $2, $3}')current_ts=$(date +%s)
# tenta con e senza anno
if ! event_ts=$(date -d "$event_date" +%s 2>/dev/null); then
    event_ts=$(date -d "$(date +%Y) $event_date" +%s 2>/dev/null || 
