...
WARN_TIMEOUT_MINUTES=5if [[ ! -r "$YUM_LOG" ]]; then    
...
# tenta con e senza annoif ! event_ts=$(date -d "$event_date" +%s 2>/dev/null); then    event_ts=$(date -d "$(date +%Y) $event_date" +%s 2>/dev/null || 
...
echo "1 PKG_INSTALL - WARN: attivit├â┬á recente ($event_date): $recent_entry"else    
