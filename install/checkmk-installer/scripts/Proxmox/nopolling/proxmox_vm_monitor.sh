...
echo "$config" | jq -r '.memory // empty')    if [[ -n "$alloc" && "$alloc" != "null" ]]; then
    ...
    current=$(pvesh get /nodes/$NODE/qemu/$vmid/status/current --output-format json 2>/dev/null)            used=$(
echo "$current" | jq -r '.mem // 0')            perc=$(( used * 100 / alloc_bytes ))            used_h=$(awk "BEGIN {printf \"%.1f\", $used / 1024 / 1024 / 1024}")            alloc_h=$(awk "BEGIN {printf \"%.1f\", $alloc_bytes / 1024 / 1024 / 1024}")            if (( perc >= CRIT_RAM )); then
...
# Salta se valori vuoti            if [[ -z "$size" || -z "$pct" || "$pct" == "-" ]]; then                continue            fi            size_gb=$(awk "BEGIN {printf \"%.1f\", $size}")            pct_int=$(awk "BEGIN {printf \"%d\", $pct}")            used_gb=$(awk "BEGIN {printf \"%.1f\", $size * $pct / 100}")            if (( pct_int >= CRIT_DISK )); then
