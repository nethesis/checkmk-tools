...
# ============================================================
# o Disinstallazione (--uninstall)
# ============================================================if [ "$1" = "--uninstall" ]; then
    echo "o Disinstallazione Checkmk Agent + FRP client..."    [ -x "$FRPC_INIT" ] && { /etc/init.d/frpc stop || true; /etc/init.d/frpc disable || true; }    killall frpc socat >/dev/null 2>&1 || true    rm -rf /etc/frp "$FRPC_BIN" "$FRPC_INIT" "$FRPC_LOG" >/dev/null 2>&1 || true    rm -f /usr/bin/check_mk_agent    rm -rf /etc/check_mk /etc/xinetd.d/check_mk >/dev/null 2>&1 || true    [ -f /etc/init.d/check_mk_agent ] && { /etc/init.d/check_mk_agent stop || true; /etc/init.d/check_mk_agent disable || true; rm -f /etc/init.d/check_mk_agent; }    
echo "Oa Disinstallazione completata."
    exit 0
fi # ============================================================
# 1 AOau Repository
# ============================================================
echo "OUO A  Configuro repository OpenWrt 23.05.0..."mkdir -p "$(dirname "$CUSTOMFEEDS")"touch "$CUSTOMFEEDS"add_repo() {    local name="$1" url="$2"    grep -q "$url" "$CUSTOMFEEDS" || 
echo "src/gz $name $url" >>"$CUSTOMFEEDS"}add_repo "openwrt_base" "$REPO_BASE"add_repo "openwrt_packages" "$REPO_PACKAGES"
# ============================================================
# 2 AOau Strumenti base
# ============================================================
echo "oa Aggiorno feed e installo tool..."opkg updateopkg install binutils tar gzip wget socat ca-certificates || opkg install busybox-fullcommand -v ar >/dev/null || { 
echo "Oi 'ar' mancante"; exit 1; }
# ============================================================
# 3 AOau Checkmk Agent
# ============================================================
echo "O14c A  Scarico Checkmk Agent..."mkdir -p "$TMPDIR"; cd "$TMPDIR"wget -q -O check-mk-agent.deb "$DEB_URL"ar x check-mk-agent.debmkdir -p data; tar -xzf data.tar.gz -C datacd data && tar -czf "$AGENT_TAR" . && cd ..
echo "oe Installo agent..."cp -f data/usr/bin/check_mk_agent /usr/bin/ 2>/dev/null || truechmod +x /usr/bin/check_mk_agentmkdir -p /etc/check_mk /etc/xinetd.dcp -rf data/etc/check_mk/* /etc/check_mk/ 2>/dev/null || truerm -rf "$TMPDIR"
echo "Oa Agent installato."
# ============================================================
# 4 AOau Servizio persistente Checkmk Agent (socat listener)
# ============================================================
echo "OUO A  Creo servizio init.d per Checkmk Agent (porta 6556, persistente con socat)..."cat >/etc/init.d/check_mk_agent <<'EOF'
...
# Auto-restart if socat crashes    procd_set_param stdout 1    procd_set_param stderr 1    procd_close_instance}

stop_service() {    
echo "Stopping Checkmk Agent..."    killall socat >/dev/null 2>&1 || true}\1
chmod +x /etc/init.d/check_mk_agent/etc/init.d/check_mk_agent enable >/dev/null 2>&1 || true/etc/init.d/check_mk_agent restartsleep 2
if pgrep -f "socat TCP-LISTEN:6556" >/dev/null 2>&1; then
    echo "Oa Checkmk Agent attivo e persistente sulla porta 6556 (socat mode)."else    
echo "OUa A  Checkmk Agent non risponde OCo verifica /usr/bin/check_mk_agent."fi
# ============================================================
# 5 AOau FRP client
# ============================================================
echo ""
...
echo "OUa A Inserisci un numero vali
do."    done
read -rp "Inserisci la chiave/token FRP: " FRP_TOKEN    
DEFAULT_NAME=$(hostname 2>/dev/null || 
echo "openwrt-host")    
echo ""    
echo "i Nome del proxy FRP (univoco per ogni host)"    
...
echo "O14c A  Scarico FRP v$FRP_VER..."    cd /tmp    
...
FRP_DIR=$(tar -tzf "$FRP_TGZ" | head -1 | cut -f1 -d"/")    mkdir -p /usr/local/bin    cp -f "$FRP_DIR/frpc" "$FRPC_BIN"    chmod +x "$FRPC_BIN"    rm -rf "$FRP_TGZ" "$FRP_DIR"    mkdir -p /etc/frp /var/log    cat >"$FRPC_CONF" <<EOF
[common]server_addr = "$SERVER_ADDR"server_port = $SERVER_PORTauth.method = "token"auth.token  = "$FRP_TOKEN"tls.enable = truelog.to = "$FRPC_LOG"log.level = "debug"[$PROXY_NAME]type        = "tcp"local_ip    = "127.0.0.1"local_port  = 6556remote_port = $REMOTE_PORTEOF    cat >"$FRPC_INIT" <<'INIT'
#!/bin/sh /etc/rc.common
START=99
STOP=10
...
stop_service() {    killall frpc >/dev/null 2>&1 || true}INIT    chmod +x "$FRPC_INIT"    /etc/init.d/frpc enable >/dev/null 2>&1 || true    /etc/init.d/frpc start || /usr/local/bin/frpc -c "$FRPC_CONF" &    sleep 2    
echo ""    if pgrep -f frpc >/dev/null 2>&1; then
    echo "Oa FRP attivo per proxy [$PROXY_NAME] Oa porta remota $REMOTE_PORT"
else        
echo "OUa A  FRP non attivo. Controlla log: tail -f $FRPC_LOG"    fi
fi
# ============================================================
# 6 AOau Fine
# ============================================================
