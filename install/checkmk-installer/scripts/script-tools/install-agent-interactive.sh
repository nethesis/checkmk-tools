...
# Wrapper: esegue la versione canonica in script-tools/full.
...
# =====================================================uninstall_frpc() {    
echo -e "\n${RED}OooOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOou${NC}"    
echo -e "${RED}Oo           DISINSTALLAZIONE FRPC CLIENT                    Oo${NC}"    
...
echo -e "\n${YELLOW}u A  Rimozione FRPC in corso...${NC}\n"        
...
# OpenWrt: init.d        if [ -f /etc/init.d/frpc ]; then
    echo -e "${YELLOW}OA A  Arresto servizio FRPC...${NC}"            /etc/init.d/frpc stop 2>/dev/null || true            /etc/init.d/frpc disable 2>/dev/null || true            rm -f /etc/init.d/frpc        fi
else        
# Linux: systemd        if systemctl is-active --quiet frpc 2>/dev/null; then
    echo -e "${YELLOW}OA A  Arresto servizio FRPC...${NC}"            systemctl stop frpc 2>/dev/null || true        fi                if systemctl is-enabled --quiet frpc 2>/dev/null; then
    echo -e "${YELLOW}OA A  Disabilito servizio FRPC...${NC}"            systemctl disable frpc 2>/dev/null || true        fi                if [ -f /etc/systemd/system/frpc.service ]; then
    echo -e "${YELLOW}u A  Rimozione file systemd...${NC}"            rm -f /etc/systemd/system/frpc.service            systemctl daemon-reload 2>/dev/null || true        fi    fi        
# Rimuovi eseguibile    if [ -f /usr/local/bin/frpc ]; then
    echo -e "${YELLOW}u A  Rimozione eseguibile...${NC}"        rm -f /usr/local/bin/frpc    fi        
# Rimuovi configurazione    if [ -d /etc/frp ]; then
    echo -e "${YELLOW}u A  Rimozione directory configurazione...${NC}"        rm -rf /etc/frp    fi        
# Rimuovi log    if [ -f /var/log/frpc.log ]; then
    echo -e "${YELLOW}u A  Rimozione file log...${NC}"        rm -f /var/log/frpc.log    fi
echo -e "\n${GREEN}Oa FRPC disinstallato completamente${NC}"    
echo -e "${CYAN}oi File rimossi:${NC}"    
echo -e "   OCo /usr/local/bin/frpc"    
echo -e "   OCo /etc/frp/"    
echo -e "   OCo /etc/systemd/system/frpc.service (Linux)"    
echo -e "   OCo /etc/init.d/frpc (OpenWrt)"    
echo -e "   OCo /var/log/frpc.log"}
# =====================================================
# Funzione: Disinstalla CheckMK Agent
# =====================================================uninstall_agent() {    
echo -e "\n${RED}OooOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOou${NC}"    
echo -e "${RED}Oo        DISINSTALLAZIONE CHECKMK AGENT                     Oo${NC}"    
echo -e "${RED}OoUOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOoEOo${NC}"        
echo -e "\n${YELLOW}u A  Rimozione CheckMK Agent in corso...${NC}\n"        
...
# OpenWrt: init.d        if [ -f /etc/init.d/check_mk_agent ]; then
    echo -e "${YELLOW}OA A  Arresto servizio agent...${NC}"            /etc/init.d/check_mk_agent stop 2>/dev/null || true            /etc/init.d/check_mk_agent disable 2>/dev/null || true            rm -f /etc/init.d/check_mk_agent        fi
else        
# Linux: systemd socket        if systemctl is-active --quiet check-mk-agent-plain.socket 2>/dev/null; then
    echo -e "${YELLOW}OA A  Arresto socket plain...${NC}"            systemctl stop check-mk-agent-plain.socket 2>/dev/null || true        fi                if systemctl is-enabled --quiet check-mk-agent-plain.socket 2>/dev/null; then
    echo -e "${YELLOW}OA A  Disabilito socket plain...${NC}"            systemctl disable check-mk-agent-plain.socket 2>/dev/null || true        fi                if [ -f /etc/systemd/system/check-mk-agent-plain.socket ]; then
    echo -e "${YELLOW}u A  Rimozione socket systemd plain...${NC}"            rm -f /etc/systemd/system/check-mk-agent-plain.socket        fi                if [ -f /etc/systemd/system/check-mk-agent-plain@.service ]; then
    echo -e "${YELLOW}u A  Rimozione service systemd plain...${NC}"            rm -f /etc/systemd/system/check-mk-agent-plain@.service        fi                systemctl daemon-reload 2>/dev/null || true    fi        
# Rimuovi eseguibile    if [ -f /usr/bin/check_mk_agent ]; then
    echo -e "${YELLOW}  A  Rimozione eseguibile agent...${NC}"        rm -f /usr/bin/check_mk_agent    fi        
# Rimuovi configurazione    if [ -d /etc/check_mk ]; then
    echo -e "${YELLOW}u A  Rimozione directory configurazione...${NC}"        rm -rf /etc/check_mk    fi        
# Rimuovi xinetd config (se presente)    if [ -f /etc/xinetd.d/check_mk ]; then
    echo -e "${YELLOW}u A  Rimozione configurazione xinetd...${NC}"        rm -f /etc/xinetd.d/check_mk        systemctl reload xinetd 2>/dev/null || true    fi
echo -e "\n${GREEN}Oa CheckMK Agent disinstallato completamente${NC}"    
echo -e "${CYAN}oi File rimossi:${NC}"    
echo -e "   OCo /usr/bin/check_mk_agent"    
echo -e "   OCo /etc/check_mk/"    
echo -e "   OCo /etc/init.d/check_mk_agent (OpenWrt)"    
echo -e "   OCo /etc/systemd/system/check-mk-agent-plain.* (Linux)"}
# =====================================================
# Gestione parametri
...
MODE="uninstall-all"        ;;    "")        
MODE="install"        ;;    *)        
echo -e "${RED}Ou Parametro non vali
