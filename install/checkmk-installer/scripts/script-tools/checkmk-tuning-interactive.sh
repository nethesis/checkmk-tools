...
# ---------------------------------------------------------------
# 1 AOau Lettura impostazioni attuali
# ---------------------------------------------------------------
echo -e "${YELLOW}Oa Lettura impostazioni correnti...${NC}"mkdir -p "$BACKUP_DIR"cp -a "$NAGIOS_CFG" "$BACKUP_DIR/" 2>/dev/nullcp -a "$GLOBAL_MK" "$BACKUP_DIR/" 2>/dev/null
...
echo "o Impostazioni attuali:"printf "  OCo max_concurrent_checks = %s\n" "$CURRENT_CONC"printf "  OCo service_check_timeout = %s\n" "$CURRENT_SERV_TMOUT"printf "  OCo host_check_timeout    = %s\n" "$CURRENT_HOST_TMOUT"printf "  OCo sleep_time            = %s\n" "$CURRENT_SLEEP"printf "  OCo inter_check_delay     = %s\n" "$CURRENT_DELAY"echogrep -E "service_check_timeout|use_cache_for_checking" "$GLOBAL_MK" 2>/dev/null && 
echo || 
echo "  Nessuna opzione extra trovata in global.mk"echo
# ---------------------------------------------------------------
# 2 AOau Inserimento nuovi valori
# ---------------------------------------------------------------
echo -e "${YELLOW}Oa Inserisci i nuovi valori (invio per usare i default suggeriti):${NC}"read -r -p "  Nuovo valore per max_concurrent_checks [30]: " NEW_CONCread -r -p "  Nuovo valore per service_check_timeout (sec) [60]: " NEW_SERV_TMOUTread -r -p "  Nuovo valore per host_check_timeout (sec) [60]: " NEW_HOST_TMOUTread -r -p "  Nuovo valore per sleep_time (sec fra i check) [0.25]: " NEW_SLEEPread -r -p "  Meto
...
# ---------------------------------------------------------------
# 3 AOau Riepilogo prima dell'applicazione
# ---------------------------------------------------------------clear
...
# ---------------------------------------------------------------
# 5 AOau Riavvio e verifica
# ---------------------------------------------------------------echo
echo -e "${YELLOW}Oa Riavvio del sito $SITE...${NC}"omd restart "$SITE"echo
