...
# ID utente per creazione note/commenti privati: "${YDEA_TOKEN_FILE:=${HOME}/.ydea_token.json}": "${YDEA_EXPIRY_SKEW:=60}": "${YDEA_DEBUG:=0}": "${YDEA_LOG_FILE:=/var/log/ydea-toolkit.log}": "${YDEA_LOG_MAX_SIZE:=10485760}"  
# 10MB: "${YDEA_LOG_LEVEL:=INFO}"  
# DEBUG, INFO, WARN, ERROR: "${YDEA_TRACKING_FILE:=/var/log/ydea-tickets-tracking.json}": "${YDEA_TRACKING_RETENTION_DAYS:=365}"  
# Mantieni ticket risolti per N giorni (1 anno)
CURL_OPTS=(  --fail-with-body  --show-error  --silent  --connect-timeout 10  --max-time 30)
# ===== Logging System =====log_rotate() {  if [[ -f "$YDEA_LOG_FILE" ]]; then    local size    size=$(stat -f%z "$YDEA_LOG_FILE" 2>/dev/null || stat -c%s "$YDEA_LOG_FILE" 2>/dev/null || 
echo 0)    if [[ "$size" -gt "$YDEA_LOG_MAX_SIZE" ]]; then      mv "$YDEA_LOG_FILE" "${YDEA_LOG_FILE}.1" 2>/dev/null || true      [[ -f "${YDEA_LOG_FILE}.1" ]] && gzip "${YDEA_LOG_FILE}.1" 2>/dev/null || true    fi  fi}log_write() {  local level="$1"; shift  local message="$*"  local timestamp  timestamp="$(date '+%Y-%m-%d %H:%M:%S')"    
# Log rotation check  log_rotate    
# Write to log file  
echo "[$timestamp] [$level] [PID:$$] $message" >> "$YDEA_LOG_FILE" 2>/dev/null || true}log_debug() {   [[ "${YDEA_DEBUG}" == "1" ]] && 
echo "­ƒöì $*" >&2 || true  log_write "DEBUG" "$*"}log_info() {   
echo "Ôä╣´©Å  $*" >&2  log_write "INFO" "$*"}log_success() {   
echo "Ô£à $*" >&2  log_write "INFO" "SUCCESS: $*"}log_warn() {  
...
# Lista tutti i ticket con filtri opzionalilist_tickets() {  local limit="${1:-50}"  local status="${2:-}"  local path="/tickets?limit=$limit"  [[ -n "$status" ]] && path="${path}&status=$status"    log_info "Recupero ticket (limit: $limit${status:+, status: $status})..."  ydea_api GET "$path"}
# Dettagli di un singolo ticketget_ticket() {  local ticket_id="$1"  [[ -n "$ticket_id" ]] || { log_error "Ticket ID richiesto"; return 2; }    log_info "Recupero ticket 
#$ticket_id..."  ydea_api GET "/tickets/$ticket_id"}
# Crea un nuovo ticketcreate_ticket() {  local title="$1"  local description="$2"  local priority="${3:-normal}"  local category_id="${4:-}"    [[ -z "$title" ]] && { log_error "Specifica almeno il titolo"; return 1; }    
# Mappa priorit├á testuale a numeri (1=bassa, 2=normale, 3=alta, 4=urgente, 5=critica)  local priority_num=2  case "${priority,,}" in    low|bassa)        priority_num=1 ;;    normal|normale)   priority_num=2 ;;    high|alta)        priority_num=3 ;;    urgent|urgente)   priority_num=4 ;;    critical|critica) priority_num=5 ;;  esac    
# Valori predefiniti da variabili ambiente o fallback  local azienda="${YDEA_AZIENDA:-2339268}"  local contatto="${YDEA_CONTATTO:-773763}"  local tipo="${YDEA_TIPO:-Nethserver}"    local body  body=$(jq -n \    --arg title "$title" \    --arg desc "${description:-}" \    --argjson prio "$priority_num" \    --argjson azienda "$azienda" \    --argjson contatto "$contatto" \    --argjson anagrafica "$azienda" \    --arg fonte "Partner portal" \    --arg tipo "$tipo" \    --arg addebito "F" \    --arg cat "$category_id" \    '{      titolo: $title,      testo: $desc,      priorita: $prio,      azienda: $azienda,      contatto: $contatto,      anagrafica_id: $anagrafica,      fonte: $fonte,      tipo: $tipo,      condizioneAddebito: $addebito    } + (if $cat != "" then {categoria: $cat} else {} end)'  )    log_info "Creazione ticket: $title (priorit├á: $priority)"  ydea_api POST "/ticket" "$body"}
# Aggiorna un ticketupdate_ticket() {  local ticket_id="$1"  local json_updates="$2"    [[ -z "$ticket_id" || -z "$json_updates" ]] && { log_error "Specifica ticket_id e json_updates"; return 1; }    log_info "Aggiornamento ticket 
#$ticket_id..."  ydea_api PATCH "/tickets/$ticket_id" "$json_updates"}
# Chiudi un ticketclose_ticket() {  local ticket_id="$1"  local note="${2:-Ticket chiuso}"    [[ -z "$ticket_id" ]] && { log_error "Specifica ticket_id"; return 1; }    local body  body=$(jq -n --arg note "$note" '{status: "closed", closing_note: $note}')    log_info "Chiusura ticket 
#$ticket_id..."  ydea_api PATCH "/tickets/$ticket_id" "$body"}
# Aggiungi commento a un ticketadd_comment() {  local ticket_id="$1"  local comment="$2"  local is_public="${3:-false}"    [[ -z "$ticket_id" || -z "$comment" ]] && {    log_error "Uso: add_comment <ticket_id> '<commento>' [pubblico:true|false]"    return 1  }    
# ID utente per campo creatoda (usa variabile personalizzabile)  local user_id="${YDEA_USER_ID_CREATE_NOTE:-4675}"    local body  body=$(jq -n \    --argjson tid "$ticket_id" \    --arg desc "$comment" \    --argjson pub "$is_public" \    --argjson uid "$user_id" \    '{ticket_id: $tid, atk: {descrizione: $desc, pubblico: $pub, creatoda: $uid}}')    log_info "Aggiunta commento a ticket 
#$ticket_id (pubblico: $is_public)..."  ydea_api POST "/ticket/atk" "$body"}
# Cerca ticket per testosearch_tickets() {  local query="$1"  local limit="${2:-20}"    [[ -z "$query" ]] && { log_error "Specifica una query di ricerca"; return 1; }    log_info "Ricerca ticket: '$query'..."  ydea_api GET "/tickets?search=$(printf %s "$query" | jq -sRr @uri)&limit=$limit"}
...
echo "$ticket_data" | jq -r '.stato // "Sconosciuto"')local titololocal titolotitolo=$(
...
# Leggi tutti i ticket non risolti  local tickets  tickets=$(jq -r '.tickets[] | select(.resolved_at == null) | .ticket_id' "$YDEA_TRACKING_FILE" 2>/dev/null || 
echo "")    if [[ -z "$tickets" ]]; then    log_info "Nessun ticket da aggiornare"    return 0  fi    while 
IFS= read -r ticket_id; do    [[ -z "$ticket_id" ]] && continue    count=$((count + 1))        log_debug "Controllo ticket 
#$ticket_id..."        
# Recupera tutti i ticket e filtra per ID (l'API non supporta filtro ?id=X)    local ticket_data    ticket_data=$(ydea_api GET "/tickets?limit=100" 2>/dev/null || 
echo "{}")        
# Filtra per ticket_id specifico e prendi il primo match    local ticket_obj    ticket_obj=$(
echo "$ticket_data" | jq --arg tid "$ticket_id" '[.objs[] | select(.id == ($tid|tonumber))] | .[0] // {}' 2>/dev/null)        if [[ "$ticket_obj" == "{}" ]] || [[ "$ticket_obj" == "null" ]]; then      log_warn "Ticket 
...
echo ""    
# YDEA_USER_ID_CREATE_TICKET  read -r -p "ID utente creazione ticket [$current_ticket_id]: " new_ticket_id  new_ticket_id="${new_ticket_id:-$current_ticket_id}"    
# YDEA_USER_ID_CREATE_NOTE  read -r -p "ID utente creazione note/commenti [$current_note_id]: " new_note_id  new_note_id="${new_note_id:-$current_note_id}"    
echo ""  
echo "­ƒôØ GESTIONE LOG E TRACKING (opzionali)"  
echo "   Configurazione avanzata per logging e monitoraggio"  
echo ""    
# Log file location  local current_log_file="${YDEA_LOG_FILE:-/var/log/ydea-toolkit.log}"  read -r -p "Percorso file log [$current_log_file]: " new_log_file  new_log_file="${new_log_file:-$current_log_file}"    
# Log max size (in MB)  local current_log_size_mb=$((${YDEA_LOG_MAX_SIZE:-10485760} / 1048576))  read -r -p "Dimensione massima log in MB [$current_log_size_mb]: " new_log_size_mb  new_log_size_mb="${new_log_size_mb:-$current_log_size_mb}"  local new_log_size=$((new_log_size_mb * 1048576))    
# Log level  local current_log_level="${YDEA_LOG_LEVEL:-INFO}"  read -r -p "Livello log (DEBUG/INFO/WARN/ERROR) [$current_log_level]: " new_log_level  new_log_level="${new_log_level:-$current_log_level}"  new_log_level=$(
echo "$new_log_level" | tr '[:lower:]' '[:upper:]')    
# Tracking file  local current_tracking_file="${YDEA_TRACKING_FILE:-/var/log/ydea-tickets-tracking.json}"  read -r -p "Percorso file tracking ticket [$current_tracking_file]: " new_tracking_file  new_tracking_file="${new_tracking_file:-$current_tracking_file}"    
# Retention days  local current_retention="${YDEA_TRACKING_RETENTION_DAYS:-365}"  read -r -p "Giorni mantenimento ticket risolti [$current_retention]: " new_retention  new_retention="${new_retention:-$current_retention}"    
echo ""  
echo "­ƒÆ¥ Salvataggio configurazione in: $env_file"    
# Crea backup se esiste  if [[ -f "$env_file" ]]; then    cp "$env_file" "${env_file}.backup.$(date +%Y%m%d_%H%M%S)"    
echo "   (backup creato: ${env_file}.backup.$(date +%Y%m%d_%H%M%S))"  fi    
# Scrivi nuovo .env  cat > "$env_file" <<EOF
# ===== YDEA TOOLKIT CONFIGURATION =====
# Generato il: $(date '+%Y-%m-%d %H:%M:%S')
# Credenziali API (OBBLIGATORIE)export 
YDEA_ID="$new_id"export 
YDEA_API_KEY="$new_key"
# ID Utente per operazioni (opzionali)export 
YDEA_USER_ID_CREATE_TICKET=$new_ticket_idexport 
YDEA_USER_ID_CREATE_NOTE=$new_note_id
# ===== GESTIONE LOG E TRACKING =====export 
YDEA_LOG_FILE="$new_log_file"export 
YDEA_LOG_MAX_SIZE=$new_log_sizeexport 
YDEA_LOG_LEVEL="$new_log_level"export 
YDEA_TRACKING_FILE="$new_tracking_file"export 
...
# export 
YDEA_DEBUG=0EOF    chmod 600 "$env_file"    
echo ""  
echo "Ô£à Configurazione salvata con successo!"  
echo ""  
echo "­ƒôØ Riepilogo:"  
echo "   YDEA_ID: $new_id"  
...
echo ""  
echo "­ƒôè Configurazione Log & Tracking:"  
...
echo ""  
echo "­ƒº¬ Test configurazione:"  
echo "   source $env_file"  
echo "   $0 login"  
echo ""}
# ===== CLI =====show_usage() {  cat >&2 <<'USAGE'­ƒøá´©Å  Ydea Toolkit - Gestione API v2SETUP:  export 
YDEA_ID="tuo_id"              
# Da: Impostazioni ÔåÆ La mia azienda ÔåÆ API  export 
YDEA_API_KEY="tua_chiave_api"    
# ID Utente per operazioni (opzionali)  export 
YDEA_USER_ID_CREATE_TICKET=4675    
# ID per creazione ticket  export 
YDEA_USER_ID_CREATE_NOTE=4675      
# ID per creazione note/commenti    export 
YDEA_DEBUG=1                  
# (opzionale) per debug verboso  export 
YDEA_LOG_FILE=/path/log.log   
# (default: /var/log/ydea-toolkit.log)COMANDI:  Autenticazione:    login                              Effettua login e salva token  API Generiche:    api <METHOD> </path> [json_body]   Chiamata API generica      Ticket - Lista e Ricerca:    list [limit] [status]              Lista ticket (default: 50)    search <query> [limit]             Cerca ticket per testo    get <ticket_id>                    Dettagli ticket specifico      Ticket - Creazione e Modifica:    create <title> [description] [priority] [category_id]    update <ticket_id> '<json>'        Aggiorna ticket (formato JSON)    close <ticket_id> [nota]           Chiudi ticket    comment <ticket_id> '<testo>'      Aggiungi commento    Tracking Ticket (Monitoraggio Stati):    track <ticket_id> <codice> <host> <service> [desc]                                       Aggiungi ticket al tracking automatico    update-tracking                    Aggiorna stati di tutti i ticket tracciati    cleanup-tracking                   Rimuovi ticket risolti vecchi    list-tracking                      Mostra JSON completo ticket tracciati    stats                              Statistiche ticket (aperti/risolti/tempi)      Log e Debug:    logs [lines]                       Mostra ultimi N log (default: 50)    clearlog                           Pulisci file di log    Configurazione:    config                             Configurazione interattiva (ID, API key, user ID)      Altro:    categories                         Lista categorie    users [limit]                      Lista utentiESEMPI:  
# Configurazione iniziale interattiva  ./ydea-toolkit.sh config    
# Login iniziale  ./ydea-toolkit.sh login  
...
# Aggiorna tutti i ticket tracciati  ./ydea-toolkit.sh update-tracking    
# Visualizza log  ./ydea-toolkit.sh logs 100  
# Chiamata API custom  ./ydea-toolkit.sh api GET /tickets/12345/history | jq .VARIABILI AMBIENTE:  
