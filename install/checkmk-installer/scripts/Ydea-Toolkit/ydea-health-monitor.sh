...
do il servizio sar├á ripristinato.---Monitor automatico Ydea HealthCheck ogni 15 minuti"            if send_email_alert "$subject" "$body"; then        log "Ô£à Notifica inviata con successo"        update_state "down" "$consecutive_failures" "true"      else        log_error "Errore invio notifica"        update_state "down" "$consecutive_failures" "false"      fi    else      
