...
echo "  Usa: sudo $0"    exit 1  fi}check_checkmk() {  if [[ ! -d "/omd/sites/${CHECKMK_SITE}" ]]; then    error "Sito CheckMK '${CHECKMK_SITE}' non trovato"    
...
NOTIFY_SCRIPT_DIR="${SCRIPT_DIR}/script-notify-checkmk"  elif [[ -d "$(dirname "${SCRIPT_DIR}")/script-notify-checkmk" ]]; then    
...
echo "  Provato: $(dirname "${SCRIPT_DIR}")/script-notify-checkmk"    exit 1  fi    info "Usando script da: ${NOTIFY_SCRIPT_DIR}"    
