...
echo    if [[ $REPLY =~ ^[Yy]$ ]]; then      mkdir -p "$YDEA_TOOLKIT_DIR"      success "Directory creata: $YDEA_TOOLKIT_DIR"    else      error "Impossibile continuare senza Ydea Toolkit"
    exit 1    fi  else    success "Ydea Toolkit trovato: $YDEA_TOOLKIT_DIR"  fi}install_scripts() {  info "Installazione script di notifica CheckMK..."    
...
# Copia ydea_realip  if [[ -f "${NOTIFY_SCRIPT_DIR}/ydea_realip" ]]; then    cp "${NOTIFY_SCRIPT_DIR}/ydea_realip" "$CHECKMK_NOTIFY_DIR/"    chmod +x "${CHECKMK_NOTIFY_DIR}/ydea_realip"    success "ydea_realip installato"  else    error "File ydea_realip non trovato in ${NOTIFY_SCRIPT_DIR}/"
    exit 1  fi    
...
# Copia health monitor (supporta sia percorso relativo che assoluto)  info "Installazione health monitor..."  local HEALTH_MONITOR  if [[ -f "${SCRIPT_DIR}/ydea-health-monitor.sh" ]]; then
    ...
    HEALTH_MONITOR="${SCRIPT_DIR}/Ydea-Toolkit/ydea-health-monitor.sh"  else    error "File ydea-health-monitor.sh non trovato"
