...
check_dependencies() {
  ...
  if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    log_info "Install with: sudo apt-get install xorriso isolinux squashfs-tools genisoimage p7zip-full wget"
    return 1
  fi
...
add_bootstrap_to_iso() {
  local iso_root="$1"
  
  log_info "Adding bootstrap script to ISO..."
  ...
  if [[ ! -f "$bootstrap_source" ]]; then
    log_error "Bootstrap script not found at: $bootstrap_source"
    return 1
  fi
...
╔══════════════════════════════════════════════════════════╗
║                                                          ║
║       CheckMK Installer - Online Edition                ║
║                                                          ║
╚══════════════════════════════════════════════════════════╝

This is a lightweight online installer that downloads the complete
CheckMK installation tools from GitHub.
...
Or the full command:
  ...
  sudo bash /cdrom/checkmk-installer/bootstrap-installer.sh

WHAT IT DOES:
-------------
1. Clones/updates repository to /opt/checkmk-tools/
...
ADVANTAGES:
-----------
- Small ISO size (base Ubuntu only)
...
For offline installation, use the full ISO instead.
EOF
  
  log_success "Bootstrap script added to ISO"
}
...
MENU TITLE CheckMK Installer Online Boot Menu

LABEL ubuntu
  MENU LABEL Boot CheckMK Online Installer (Ubuntu Live)
...
#!/bin/bash
# CheckMK Online Installer Autostart
...
customize_grub() {
  ...
  # Update menu title
  sed -i 's/Ubuntu/CheckMK Installer Online/g' "$grub_cfg"
...
build_iso() {
  ...
  if [[ -n "$efi_boot_args" ]]; then
    ...
    xorriso -as mkisofs \
      ...
      -volid "CHECKMK_ONLINE" \
      -appid "CheckMK Online Installer" \
      -publisher "CheckMK Tools" \
      -preparer "makeiso-online.sh" \
...
main() {
  log_info "Starting online ISO build process..."
  ...
  # Add bootstrap script (lightweight)
  if ! add_bootstrap_to_iso "$iso_root"; then
    exit 1
  fi
  ...
  echo ""
  display_box "Online ISO Build Complete!" \
    "" \
    "ISO Location: $output_iso" \
    "ISO Size: $iso_size" \
    "" \
    "This is a LIGHTWEIGHT online installer that:" \
    "  - Requires internet connection" \
