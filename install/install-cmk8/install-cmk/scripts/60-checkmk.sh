...
# Chiedi l'URL se non presenteif [[ -z "${CHECKMK_DEB_URL:-}" ]]; then  read -rp "├░┼©ÔÇÿÔÇ░ Inserisci l'URL completo del pacchetto Checkmk (.deb): " CHECKMK_DEB_URLfiif [[ -z "${CHECKMK_DEB_URL:-}" ]]; then  
echo "├ó┬Ø┼Æ Nessun URL fornito. Interrompo."  exit 1fi
echo "==> Scarico da: $CHECKMK_DEB_URL"wget -O "$DEB_PATH" "$CHECKMK_DEB_URL"
echo "==> Installo $DEB_PATH"apt-get update -yif ! apt-get install -y "$DEB_PATH"; then  
echo "==> Risolvo dipendenze (apt -f install)"  apt-get -f install -y  apt-get install -y "$DEB_PATH"fi
# Crea il site se non esisteif ! omd sites | awk '{print $1}' | grep -qx "$SITE"; then  
