...
# Esecuzione challenge opzionaleread -r -p "Vuoi eseguire subito la challenge Let's Encrypt per ottenere il certificato? (s/n): " RUN_CHALLENGEif [[ "$RUN_CHALLENGE" =~ ^[sS]$ ]]; then  if [[ -z "$LETSENCRYPT_DOMAINS" ]]; then    
