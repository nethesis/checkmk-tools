...
echo "chrony installato: salto systemd-timesyncd."  exit 0fi
if dpkg -s ntp >/dev/null 2>&1; then  
echo "ntp installato: salto systemd-timesyncd."  exit 0fiapt-get update -yapt-get install -y systemd-timesyncdmkdir -p /etc/systemd/timesyncd.conf.d
