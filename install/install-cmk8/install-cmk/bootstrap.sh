...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"cd "$SCRIPT_DIR"if [[ -f .env ]]; then  set -a  source .env  set +a
else  
...
do."  exit 1fiexport TIMEZONE SSH_PORT PERMIT_ROOT_LOGIN CLIENT_ALIVE_INTERVAL CLIENT_ALIVE_COUNTMAX LOGIN_GRACE_TIME ROOT_PASSWORD OPEN_HTTP_HTTPS LETSENCRYPT_EMAIL LETSENCRYPT_DOMAINS WEBSERVER NTP_SERVERS CHECKMK_ADMIN_PASSWORD CHECKMK_DEB_URLrun(){ 
