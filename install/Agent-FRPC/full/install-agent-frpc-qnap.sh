...
: <<'CORRUPTED_afd7b5559bab4f54b351367123825e4e'
#!/bin/bash
# =====================================================
...
echo -e "   QTS Version: ${CYAN}$QTS_VERSION${NC}"
    ...
    NAS_TYPE="nethesis"        
echo -e "${GREEN}Ô£ô Nethesis NAS rilevato${NC}"        if [ -f /etc/nethserver-release ]; then            cat /etc/nethserver-release        fi
elif [ -d /share/CACHEDEV1_DATA ] || [ -d /share/MD0_DATA ]; then
    NAS_TYPE="qnap-like"        
echo -e "${GREEN}Ô£ô Sistema compatibile QNAP rilevato${NC}"
else        
...
# Verifica wget    if ! command -v wget >/dev/null 2>&1; then
    echo -e "${YELLOW}ÔÜá´©Å  wget non trovato, tentativo installazione...${NC}"        if command -v opkg >/dev/null 2>&1; then            opkg update && opkg install wget
elif command -v yum >/dev/null 2>&1; then            yum install -y wget
elif command -v apt-get >/dev/null 2>&1; then            apt-get update && apt-get install -y wget
else            
...
do usato    if [ "${USE_STANDALONE_MODE:-false}" = "true" ]; then
        if is_process_running "agent_daemon.sh" || is_process_running "nc.*6556"; then
    echo -e "${GREEN}Ô£ô CheckMK Agent attivo su porta 6556 (standalone daemon)${NC}"
else            
echo -e "${YELLOW}ÔÜá´©Å  Agent daemon potrebbe non essere attivo${NC}"        fi
else        if is_process_running "socat.*6556"; then
    echo -e "${GREEN}Ô£ô CheckMK Agent attivo su porta 6556 (socat)${NC}"
else            
...
# Avvia FRPC se richiesto    if [ "$INSTALL_FRPC" = "yes" ]; then
    ...
    echo -e "${GREEN}Ô£ô FRPC Client attivo${NC}"
else            
...
echo -e "\n${CYAN}­ƒôè Test CheckMK Agent locale:${NC}"    if /usr/bin/check_mk_agent | head -n 10; then
    echo -e "${GREEN}Ô£ô Agent risponde correttamente${NC}"
else        
...
echo "exit" | timeout 2 nc localhost 6556 2>/dev/null | head -n 3; then
    echo -e "${GREEN}Ô£ô Porta 6556 accessibile${NC}"
else        
...
do di avvio usato    if [ "${USE_STANDALONE_MODE:-false}" = "true" ]; then
    echo -e "   ÔÇó Meto
do avvio: ${YELLOW}standalone daemon (netcat/bash)${NC}"
else        
echo -e "   ÔÇó Meto
