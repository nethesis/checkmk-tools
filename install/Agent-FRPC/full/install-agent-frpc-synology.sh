...
echo "Agent gi├á in esecuzione (PID: $OLD_PID)"
    exit 0    else        rm -f "$PID_FILE"    fi
fi
# Verifica se la porta ├¿ gi├á in uso
if netstat -tuln 2>/dev/null | grep -q ":$PORT "; then    log_msg "ERRORE: Porta $PORT gi├á in uso"    
echo "ERRORE: Porta $PORT gi├á in uso"
    exit 1filog_msg "Avvio CheckMK Agent daemon sulla porta $PORT"
...
echo "ERRORE: Python non disponibile e socat non installato"
    exit 1fiDAEMON_EOF        chmod +x "$AGENT_DIR/agent_daemon.sh"        
...
echo "Usage: $0 {start|stop|restart}"
    exit 1        ;;esacexit 0EOF        chmod +x "$AUTOSTART_SCRIPT"        
...
FRP_DIR_NAME=$(find . -maxdepth 1 -type d -name "frp_*" | head -1)        if [ -z "$FRP_DIR_NAME" ] || [ ! -f "$FRP_DIR_NAME/frpc" ]; then
    echo -e "${RED}Ô£ù Errore nell'estrazione di FRPC${NC}"
    exit 1    fi
...
echo -n "Token autenticazione: "    read -r FRP_TOKEN    if [ -z "$FRP_TOKEN" ]; then
    echo -e "${RED}Ô£ù Token obbligatorio${NC}"
    exit 1    fi        
# Nome host/client    
...
if [ -f "$PID_FILE" ]; then
    ...
    echo "FRPC gi├á in esecuzione (PID: $OLD_PID)"
    exit 0    else        rm -f "$PID_FILE"    fi
fi
...
echo "Failed to start FRPC"    rm -f "$PID_FILE"
    exit 1fiEOF        chmod +x "$FRPC_DIR/start_frpc.sh"        
