...
echo ""    local choice  while true; do    read -r -p "Select installation type (1-2): " choice    case "$choice" in      1)        
echo "server"        return 0        ;;      2)        
echo "client"        return 0        ;;      *)        
echo "Invalid choice. Please select 1 or 2."        ;;    esac  done}
#
#
#
#
...
echo ""    local bind_port  local token  local dashboard_port  local dashboard_user  local dashboard_pwd  local enable_tls  local cert_file  local key_file    read -r -p "Bind port [7000]: " bind_port  bind_port="${bind_port:-7000}"    read -r -p "Authentication token: " token    read -r -p "Dashboard port [7500]: " dashboard_port  dashboard_port="${dashboard_port:-7500}"    read -r -p "Dashboard username [admin]: " dashboard_user  dashboard_user="${dashboard_user:-admin}"    read -r -p "Dashboard password: " dashboard_pwd    
echo ""  read -r -p "Enable TLS? (y/n) [n]: " enable_tls  enable_tls="${enable_tls:-n}"    if [[ "$enable_tls" == "y" ]]; then    read -r -p "TLS certificate file path: " cert_file    read -r -p "TLS key file path: " key_file  fi    
...
#
#
#configure_frpc() {  log_info "Configuring FRPC client..."    
# Ask for configuration  
echo ""  
echo "${YELLOW}FRPC Client Configuration${NC}"  
echo ""    local server_addr  local server_port  local token  local checkmk_remote_port  local ssh_remote_portlocal hostnamelocal hostnamehostname=$(hostname)    read -r -p "FRP server address: " server_addr    read -r -p "FRP server port [7000]: " server_port  server_port="${server_port:-7000}"    read -r -p "Authentication token: " token    read -r -p "Remote port for CheckMK agent (6556): " checkmk_remote_port    
echo ""  read -r -p "Enable SSH tunnel? (y/n) [n]: " enable_ssh  enable_ssh="${enable_ssh:-n}"    if [[ "$enable_ssh" == "y" ]]; then    read -r -p "Remote port for SSH (22): " ssh_remote_port  fi    
# Create config directory  mkdir -p "$FRP_CONFIG_DIR"    local config_file="$FRP_CONFIG_DIR/frpc.toml"    
# Create FRPC configuration  cat > "$config_file" <<EOF
# FRPC Client Configuration for $hostnameserverAddr = "$server_addr"serverPort = $server_port
# Authenticationauth.method = "token"auth.token = "$token"
# Logginglog.to = "$FRP_LOG_DIR/frpc.log"log.level = "info"
# CheckMK Agent Proxy[[proxies]]name = "checkmk-$hostname"type = "tcp"localIP = "127.0.0.1"localPort = 6556remotePort = $checkmk_remote_porttransport.useEncryption = truetransport.useCompression = trueEOF  if [[ "$enable_ssh" == "y" ]] && [[ -n "$ssh_remote_port" ]]; then    cat >> "$config_file" <<EOF
# SSH Tunnel[[proxies]]name = "ssh-$hostname"type = "tcp"localIP = "127.0.0.1"localPort = 22remotePort = $ssh_remote_porttransport.useEncryption = truetransport.useCompression = trueEOF  fi    chmod 600 "$config_file"    log_success "FRPC configured: $config_file"    
# Store config for later display  
