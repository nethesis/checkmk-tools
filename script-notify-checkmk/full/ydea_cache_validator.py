...
def check_ticket_exists(ticket_id: int) -> bool:
    ...
    exitcode, stdout, stderr = toolkit_cmd(['get', str(ticket_id)])
    ...
    # Other errors (500, timeout) -> assume exists to be safe
    log(f"WARN: API check failed for #{ticket_id} (code {exitcode}), assuming valid.")
    return True
