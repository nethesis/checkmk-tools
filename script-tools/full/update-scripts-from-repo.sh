...
#
# --auto: modalitÔö£├í automatica, cerca in tutto il sistema (/opt, /usr/local, /home)
#set -e
# Colori per output
RED='\033[0;31m'
GREEN='\033[0;32m'
...
echo -e "${GREEN}├ö┬ú├┤${NC} $1"}log_warning() {    
echo -e "${YELLOW}├ö├£├í${NC} $1"}log_error() {    
echo -e "${RED}├ö┬ú├╣${NC} $1"}
# Parametri
REPO_DIR="${1:-/opt/checkmk-tools}"
AUTO_MODE=false
# Check per modalitÔö£├í auto
...
if $AUTO_MODE; then    log "ModalitÔö£├í: AUTOMATICA (sistema completo)"    log "Ricerca in: / (tutto il filesystem)"    log ""    log "├ö├£├í┬┤┬®├à  ATTENZIONE: La scansione completa puÔö£Ôûô richiedere alcuni minuti"
else    log "ModalitÔö£├í: MANUALE"    log "Ricerca in: ${SEARCH_PATHS[0]}"filog ""
...
# Salta se giÔö£├í Ôö£┬┐ una versione "r*"        if [[ "$script_name" =~ ^r.* ]]; then            continue        fi                
...
# Escludi directory di sistema per scansione root            find / \( -path /proc -o -path /sys -o -path /dev -o -path /run -o -path /tmp -o -path /var/tmp -o -path '*/snap' -o -path '*/.git' \) -prune -o -type f \( -name "*.sh" -o -executable \) -print0 2>/dev/null
else            find "$search_dir" -type f \( -name "*.sh" -o -executable \) -print0 2>/dev/null        fi    )donelog ""log "========================================"log "RIEPILOGO AGGIORNAMENTO"log "========================================"log_success "Aggiornati: $UPDATED script"log_warning "Trovati ma non aggiornati: $SKIPPED script"
...
echo "  ├ö├ç├│ $original ├ö├Ñ├å ${REPLACEMENTS[$original]}"    done    log ""        log "========================================"    log "VERIFICA FILE SOSTITUITI"    log "========================================"    log "Controllo presenza e integritÔö£├í dei file aggiornati..."    log ""        verify_success=0    verify_failed=0        for original in "${!REPLACEMENTS[@]}"; do        if [[ -f "$original" ]]; then            
# Verifica che sia eseguibile            if [[ -x "$original" ]]; then                
# Verifica che sia un file bash vali
do (se Ôö£┬┐ uno script bash)                if head -1 "$original" 2>/dev/null | grep -q "bash"; then
                    if bash -n "$original" 2>/dev/null; then                        log_success "OK: $original (presente, eseguibile, sintassi valida)"                        ((verify_success++))                    else                        log_error "ERRORE SINTASSI: $original"                        ((verify_failed++))                    fi
else                    log_success "OK: $original (presente, eseguibile)"                    ((verify_success++))                fi
else                log_warning "WARN: $original (presente ma non eseguibile)"                ((verify_failed++))            fi
else            log_error "MANCANTE: $original"            ((verify_failed++))        fi    done        log ""    log "Verifica completata: ${verify_success} OK, ${verify_failed} problemi"    log ""    log "Backup salvato in: $BACKUP_DIR"    log ""    log_success "├ö┬ú├┤ Aggiornamento completato!"    log ""    log "Per ripristinare il backup:"    log "  cp -r $BACKUP_DIR/* /"else    log_warning "Nessuno script aggiornato"    rm -rf "$BACKUP_DIR" 2>/dev/null || truefilog ""exit 0
