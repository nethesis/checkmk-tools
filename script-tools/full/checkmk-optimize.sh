...
echo "[$(date +%F_%T)] ERRORE creazione snapshot PRE: $SNAP_COMMENT" >> "$TSLOG"    fi  fielse  log "Timeshift non installato, nessuno snapshot creato."fi
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 2. Disabilita servizi inutiliread -p "Disabilitare servizi non essenziali (snapd, apport, motd-news)? (s/n): " opt_servicesif [[ "$opt_services" =~ ^[Ss]$ ]]; then  for svc in snapd.service apport.service motd-news.timer; do    systemctl disable --now $svc 2>/dev/null && log "Disabilitato: $svc"  donefi
# ------------------------------------------------------------
...
echo "[$(date +%F_%T)] ERRORE creazione snapshot POST: $SNAP_COMMENT" >> "$TSLOG"    fi  fifi
# ------------------------------------------------------------
