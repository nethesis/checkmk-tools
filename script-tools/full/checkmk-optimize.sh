...
# ------------------------------------------------------------
# 1. Ottimizzazione SWAP e ZRAMread -p "Ottimizzare SWAP (swappiness, zram)? (s/n): " opt_swap
if [[ "$opt_swap" =~ ^[Ss]$ ]]; then  log "Backup sysctl.conf e configurazione zram..."  cp -a /etc/sysctl.conf "$BACKUP_DIR/sysctl.conf.$DATE.bak"  
...
# ------------------------------------------------------------
# 2. Disabilita servizi inutiliread -p "Disabilitare servizi non essenziali (snapd, apport, motd-news)? (s/n): " opt_services
if [[ "$opt_services" =~ ^[Ss]$ ]]; then  for svc in snapd.service apport.service motd-news.timer; do    systemctl disable --now $svc 2>/dev/null && log "Disabilitato: $svc"  done
fi
# ------------------------------------------------------------
...
if [[ "$opt_apache" =~ ^[Ss]$ ]]; then  mkdir -p /etc/systemd/system/apache2.service.d/  cat <<EOT > /etc/systemd/system/apache2.service.d/limits.conf[Service]Limit
