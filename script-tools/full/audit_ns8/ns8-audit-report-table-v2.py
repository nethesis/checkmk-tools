#!/usr/bin/env python3
"""
ns8-audit-report-table-v2.py - Report NS8 con Formato Tabelle Compatte

Report completo ambiente NethServer 8 con visualizzazione tabellare:
  1) Utenti Active Directory (Samba)
  2) Stato account utenti AD (data creazione, abilitato/disabilitato)
  3) Scadenze password utenti AD (tabella 4 colonne)
  4) Gruppi AD con membri (tabella 2 colonne, una riga per membro)
  5) Permessi share di rete con ACL completi (tabella 3 colonne)
  6) Condivisioni WebTop (tabella 3 colonne)
...
def parse_ldap_timestamp(ts: str) -> Optional[str]:
    """
    Parse LDAP/AD timestamp to YYYY-MM-DD.
    
    Handles formats:
      - '20250115120000.0Z'  (whenCreated/whenChanged)
      - '20250115120000Z'
    ...
    """
    ts = ts.strip()
    for fmt in ("%Y%m%d%H%M%S.%fZ", "%Y%m%d%H%M%SZ", "%Y%m%d%H%M%S.%f%z"):
        try:
...
def collect_user_status_table(samba_module: str, output_dir: Path) -> int:
    """
    Collect AD user creation date and enabled/disabled status.
    
    For each user, queries samba-tool user show to extract:
      - whenCreated: account creation date
      - userAccountControl: bit 0x0002 = ACCOUNTDISABLE
      - whenChanged: last modification (approximate disabled-since if disabled)
    
    Returns:
        Number of disabled users found
    """
    log_info("Raccolta stato account utenti AD...")
    ...
    for idx, username in enumerate(users, 1):
        ...
        if idx % 10 == 0:
            log_info(f"  Progress: {idx}/{len(users)} utenti elaborati...")
        ...
        if exit_code != 0:
            status_data.append((username, "N/A", "Errore query", ""))
            continue
        
        # Parse whenCreated
        created_match = re.search(r'^whenCreated:\s+(.+)$', stdout, re.MULTILINE)
        created_str = "N/A"
        if created_match:
            parsed = parse_ldap_timestamp(created_match.group(1))
            if parsed:
                created_str = parsed
        
        # Parse userAccountControl
        uac_match = re.search(r'^userAccountControl:\s+(\d+)', stdout, re.MULTILINE)
        is_disabled = False
        if uac_match:
            uac_val = int(uac_match.group(1))
            is_disabled = bool(uac_val & 0x0002)
        
        # Parse whenChanged (approximate disabled-since if disabled)
        changed_match = re.search(r'^whenChanged:\s+(.+)$', stdout, re.MULTILINE)
        changed_str = ""
        if changed_match:
            parsed = parse_ldap_timestamp(changed_match.group(1))
            if parsed:
                changed_str = parsed
    ...
    for username, created, status, disabled_date in status_data:
        ...
        if status == "Disabilitato":
            ...
            status_display = f"{GREEN}{status}{NC}"
        else:
    ...
    print()
    print("=" * 120)
    log_success(f"Stato account completato ({len(status_data)} utenti, {disabled_count} disabilitati)")
    
    # Write MD file
    md_file = output_dir / "06_user_status.md"
    with open(md_file, 'w', encoding='utf-8') as f:
        f.write("# Stato Account Utenti AD\n\n")
    ...
    global GLOBAL_DISABLED_COUNT
    GLOBAL_DISABLED_COUNT = disabled_count
...
def generate_summary_table(output_dir: Path) -> None:
    ...
    global GLOBAL_USER_COUNT, GLOBAL_GROUP_COUNT, GLOBAL_COMPUTER_COUNT, GLOBAL_SHARE_COUNT, GLOBAL_WEBTOP_COUNT, GLOBAL_DISABLED_COUNT
    ...
    print(f"{'Utenti Disabilitati':<30} {GLOBAL_DISABLED_COUNT:<20}")
    print()
    print("=" * 80)
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Utenti Disabilitati: {GLOBAL_DISABLED_COUNT}\n")
...
Utenti Disabilitati:    {GLOBAL_DISABLED_COUNT}
...
- 06_user_status.md (stato account utenti)
"""
