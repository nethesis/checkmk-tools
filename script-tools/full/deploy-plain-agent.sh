...
echo "Usa $0 --force se vuoi sovrascriverli."
    exit 1
fi echo "├░┼©ÔÇÿÔÇ░ Disabilito agent controller TLS (cmk-agent-ctl-daemon)..."systemctl stop cmk-agent-ctl-daemon 2>/dev/null || truesystemctl disable cmk-agent-ctl-daemon 2>/dev/null || true
