...
# smart-wrapper-template.sh
# Template di wrapper "smart" per script Checkmk.
# - Tenta update da URL (curl) con timeout
# - Usa cache locale come fallback
# - Se manca la cache: output CRITICAL in formato local check

SCRIPT_NAME="check_example"
SCRIPT_TYPE="local"   # local|spool|plugin|notification
GITHUB_URL="https://example.invalid/path/to/script.sh"
TIMEOUT_SECONDS=5
