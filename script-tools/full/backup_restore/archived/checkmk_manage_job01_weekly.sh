#!/bin/bash
# checkmk_manage_job01_weekly.sh
# Gestisce backup settimanale normale CheckMK (job01-complete)
# - Upload diretto senza compressione (362M)
# - Upload a cloud DigitalOcean Spaces
# - Retention: 5 backup (locale + cloud)
#
# Uso: ./checkmk_manage_job01_weekly.sh
...
# Step 1: Trova backup job01-complete SENZA timestamp (ancora da processare)
log "📂 Searching for unprocessed job01-complete backup..."
BACKUP=$(find "$BACKUP_DIR" -maxdepth 1 -type d -name "*job01-complete" -not -name "*-[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]-*" | head -1)
...
BACKUP_SIZE=$(du -h "$SITE_TAR" | cut -f1)
log "📦 Backup size: $BACKUP_SIZE"

# Verifica se già rinominato (ha timestamp)
...
# Step 2: Upload a cloud (senza compressione)
...
# Step 3: Retention locale (mantieni ultimi 5)
log "🗂️  Applying local retention (keep last $RETENTION_LOCAL)..."
BACKUPS_LOCAL=($(find "$BACKUP_DIR" -maxdepth 1 -type d -name "*job01-complete*" | sort -r))
...
# Step 4: Retention cloud (mantieni ultimi 5)
...
log "Size: $BACKUP_SIZE (full, uncompressed)"
