...
echo -e "${R} VPS automation secret is required${N}"        exit 1    fi        su
do -u "$site" bash <<EOFset -e
# Enter OMD environmentcd /omd/sites/$sitesource .profile
...
echo -e "${B} Audit: Quick Diagnostics ${N}"    local site    if [[ "$ROLE" == "vps" ]]; then        site="$VPS_SITE"    else        site="$LOCAL_SITE"    fic/check_mk/multisite.d/wato/distributed_monitoring.mk || true
fi
...
do $0${N}"        exit 1    fi        case "$ROLE" in        central)            configure_central            ;;        remote)            configure_remote            ;;        *)            
...
echo -e "${R} This script requires root privileges${N}"        
echo -e "${Y} Run with: su
do $0${N}"        exit 1    fi        case "$ROLE" in        vps)            configure_vps            ;;        local)            configure_local            ;;        *)            
echo -e "${R} Could not detect role${N}"            
