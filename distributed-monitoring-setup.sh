...
echo -e "\n${Y} Checking current configuration...${N}"        if sudo -u "$site" test -f "etc/check_mk/multisite.d/wato/distributed_monitoring.mk"; then        
echo -e "${G} Distributed configuration already exists${N}"        if sudo -u "$site" grep -q "replication.*push" "etc/check_mk/multisite.d/wato/distributed_monitoring.mk" 2>/dev/null; then            
...
# Switch to site user    
echo -e "\n${Y} Configuring distributed monitoring on central site...${N}"        sudo -u "$site" bash <<EOFset -e
...
echo -e "Run on VPS: ${B}sudo -u $VPS_SITE cat ~/var/check_mk/web/automation/automation.secret${N}"    
...
echo -e "${R} VPS automation secret is required${N}"        exit 1    fi        sudo -u "$site" bash <<EOFset -e
# Enter OMD environmentcd /omd/sites/$sitesource .profile
...
echo "wato_enabled = True" >> etc/check_mk/multisite.mkfi
# Restart site
...
echo -e "${G} Site '$site' found${N}"        
# Switch to site user    
echo -e "\n${Y} Configuring distributed monitoring on remote site...${N}"        sudo -u "$site" bash <<EOFset -e
# Enter OMD environmentcd /omd/sites/$sitesource .profile
#
#
#
...
echo -e "${G} Site '$site' found${N}"        
# Switch to site user    
echo -e "\n${Y} Configuring VPS as remote receiver...${N}"        sudo -u "$site" bash <<EOFset -e
# Enter OMD environmentcd /omd/sites/$sitesource .profile
...
echo -e "${B}Site:${N} $site (${G}$ROLE${N})"    sudo -u "$site" bash -lc 'set -ecd /omd/sites/'"$site"'source .profile
...
echo -e "${B} Audit: Quick Diagnostics ${N}"    local site    if [[ "$ROLE" == "vps" ]]; then        site="$VPS_SITE"    else        site="$LOCAL_SITE"    fic/check_mk/multisite.d/wato/distributed_monitoring.mk || truefi
...
# Check root/sudo for configuration modes    if [[ $EUID -ne 0 ]] && ! sudo -n true 2>/dev/null; then        
echo -e "${R} This script requires root privileges${N}"        
echo -e "${Y} Run with: sudo $0${N}"        exit 1    fi        case "$ROLE" in        central)            configure_central            ;;        remote)            configure_remote            ;;        *)            
echo -e "${R} Could not detect role${N}"            
echo -e "${Y} Set ROLE environment variable: 
ROLE=central or 
ROLE=remote${N}"            exit 1            ;;    esacmain() {    
# Mode switches    if [[ "${1:-}" == "--audit" || "${1:-}" == "audit" ]]; then        audit        return    fi    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then        
echo "Usage: $0 [--audit]"        
...
# Check root/sudo for configuration modes    if [[ $EUID -ne 0 ]] && ! sudo -n true 2>/dev/null; then        
echo -e "${R} This script requires root privileges${N}"        
echo -e "${Y} Run with: sudo $0${N}"        exit 1    fi        case "$ROLE" in        vps)            configure_vps            ;;        local)            configure_local            ;;        *)            
echo -e "${R} Could not detect role${N}"            
echo -e "${Y} Set VPS_HOST environment variable or run manually:${N}"            
echo -e "  ${B}
ROLE=vps $0${N}  (on VPS)"            
echo -e "  ${B}
ROLE=local $0${N}  (on local box)"            exit 1            ;;    esac        
