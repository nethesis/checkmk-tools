...
if [[ -n "$TICKET_ID_2" && "$TICKET_ID_2" != "null" ]]; then
  ...
  if [[ "$SLA_FOUND" == "$SLA_ID" ]]; then
    ...
    echo "⚠️ SLA diversa: trovato ID=$SLA_FOUND, atteso ID=$SLA_ID"
  fi
else
  echo "❌ Creazione ticket fallita"
  echo ""
...
if [[ -z "$CONTRACT_ID" || "$CONTRACT_ID" == "null" ]]; then
  echo "⚠️ NESSUN CONTRATTO trovato per questa anagrafica"
  echo "   Skippo TEST 3"
  echo ""
else
  ...
  if [[ -n "$TICKET_ID_3" && "$TICKET_ID_3" != "null" ]]; then
    ...
    if [[ "$CONTRACT_FOUND" == "$CONTRACT_ID" ]]; then
      ...
      echo "⚠️ Contratto diverso o mancante: trovato ID=$CONTRACT_FOUND, atteso ID=$CONTRACT_ID"
    fi
    
    if [[ -n "$SLA_FOUND" && "$SLA_FOUND" != "null" ]]; then
      echo "✅ SLA estratta dal contratto: ID=$SLA_FOUND"
    else
      echo "⚠️ SLA NON estratta dal contratto"
    fi
  else
    echo "❌ Creazione ticket fallita"
    echo ""
...
echo ""
echo "⚠️ NOTA: Se YDEA non ha documentato gli endpoint, questi sono i campi testati:"
