...
NOW=$(date +"%Y-%m-%d %H:%M:%S")mkdir -p /var/lib/check_mk_agent
# ultima riga processata
LAST_LINE=0[ -f "$STATEFILE" ] && 
LAST_LINE=$(cat "$STATEFILE")
# nuove righe cockpit
NEW_LINES=$(awk -v last="$LAST_LINE" 'NR>last && /cockpit-ws:/ {print NR " " $0}' "$LOGFILE")
# aggiorna puntatoreif [ -n "$NEW_LINES" ]; then    
NEW_LAST=$(
echo "$NEW_LINES" | tail -n1 | awk '{print $1}')    
echo "$NEW_LAST" > "$STATEFILE"    while read -r _nr line; do        if [[ "$line" =~ "New connection to session from" ]]; then            ip=$(
...
echo "0 $SERVICE - $NOW cockpit logout from $ip"        fi    done <<< "$NEW_LINES"else    
# info di stato (non notifica perch├® rimane 0)    
ACTIVE=$(ss -tnp 2>/dev/null | grep -c cockpit-ws)    
echo "0 $SERVICE - $ACTIVE cockpit session(s) active"fi
