...
if [[ ! -r "$YUM_LOG" ]]; then    
echo "2 PKG_INSTALL - CRITICAL: $YUM_LOG non leggibile"    exit 2fimkdir -p /var/lib/check_pkg_install >/dev/null 2>&1 || trueevent_line=$(grep -E 'Installed:|Updated:|Erased:|Removed:' "$YUM_LOG" | tail -n 1)
