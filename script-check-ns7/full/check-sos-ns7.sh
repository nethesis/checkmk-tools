...
echo 1 || 
echo 0}vpn_status=$(is_active "$VPN_UNIT")ssh_status=$(is_active "$SSH_UNIT")if [[ "$vpn_status" -eq 1 && "$ssh_status" -eq 1 ]]; then    state=1    msg="SOS attivo"
elif [[ "$vpn_status" -eq 0 && "$ssh_status" -eq 0 ]]; then    state=0    msg="SOS disattivo"
else    state=2    msg="SOS PARZIALE: 
VPN=${vpn_status} 
SSH=${ssh_status}"fi
