...
# Funzione per leggere statistiche interfacciaget_stats() {    local device="$1"    if [[ -d "/sys/class/net/$device" ]]; then
...
# Monitora WANfor iface in $wan_ifaces; do    device=$(get_device "$iface")    if [[ -n "$device" ]]; then        read -r rx_bytes tx_bytes rx_packets tx_packets rx_errors tx_errors < <(get_stats "$device")                status=0        if [[ $rx_errors -gt 100 || $tx_errors -gt 100 ]]; then
    status=1        fi
echo "$status ${iface}_traffic - RX: $rx_bytes bytes, TX: $tx_bytes bytes | rx_bytes=$rx_bytes tx_bytes=$tx_bytes rx_packets=$rx_packets tx_packets=$tx_packets rx_errors=$rx_errors tx_errors=$tx_errors"    fi
done
# Monitora LANfor iface in $lan_ifaces; do    device=$(get_device "$iface")    if [[ -n "$device" ]]; then        read -r rx_bytes tx_bytes rx_packets tx_packets rx_errors tx_errors < <(get_stats "$device")                status=0        if [[ $rx_errors -gt 100 || $tx_errors -gt 100 ]]; then
    status=1        fi
echo "$status ${iface}_traffic - RX: $rx_bytes bytes, TX: $tx_bytes bytes | rx_bytes=$rx_bytes tx_bytes=$tx_bytes rx_packets=$rx_packets tx_packets=$tx_packets rx_errors=$rx_errors tx_errors=$tx_errors"    fi
done
