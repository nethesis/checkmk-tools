...
echo "1 DHCP_Leases - File leases non trovato"
    exit 0
fi # Conta lease attivi (non expired)current_time=$(date +%s)active_leases=0expired_leases=0total_leases=0while 
IFS=' ' read -r expire_time mac ip hostname client_id; do    total_leases=$((total_leases + 1))        if [[ $expire_time -gt $current_time ]]; then
    active_leases=$((active_leases + 1))    else        expired_leases=$((expired_leases + 1))    fi
done < "$LEASE_FILE"
...
if [[ $max_leases -gt 0 ]]; then
    percent=$((active_leases * 100 / max_leases))else    percent=0
fi # Determina stato
