...
echo "<<<firewall_rules>>>"
# Verifica che iptables sia runningif ! command -v iptables >/dev/null 2>&1; then    
...
# Stato in base a regole attiveif [[ $total_rules -eq 0 ]]; then    status=2    status_text="CRITICAL - Nessuna regola attiva"elif [[ $total_rules -lt 5 ]]; then    status=1    status_text="WARNING - Poche regole attive"else    status=0    status_text="OK"fi
