...
# Determina stato in base al load (warning se > 0.8 per CPU, critical se > 1.5)load1_check=$(awk "BEGIN {if ($load1_norm > 1.5) print 2; else if ($load1_norm > 0.8) print 1; else print 0}")status=$load1_checkif [[ $status -eq 2 ]]; then    status_text="CRITICAL - Load alto"elif [[ $status -eq 1 ]]; then    status_text="WARNING - Load elevato"else    status_text="OK"fi
