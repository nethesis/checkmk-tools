...
# Determina stato (warning 80%, critical 90%)if [[ $percent -ge 90 ]]; then    status=2    status_text="CRITICAL"elif [[ $percent -ge 80 ]]; then    status=1    status_text="WARNING"else    status=0    status_text="OK"fi
# Output CheckMK con perfdata
